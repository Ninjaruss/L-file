...
export class AddFluxerFields1740000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "user" ADD COLUMN IF NOT EXISTS "fluxerId" VARCHAR`,
    );
    await queryRunner.query(
      `ALTER TABLE "user" ADD COLUMN IF NOT EXISTS "fluxerUsername" VARCHAR`,
    );
    await queryRunner.query(
      `ALTER TABLE "user" ADD COLUMN IF NOT EXISTS "fluxerAvatar" VARCHAR`,
    );
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_user_fluxerId" ON "user" ("fluxerId")`,
    );

    await queryRunner.query(
      `ALTER TYPE "public"."user_profilepicturetype_enum" ADD VALUE IF NOT EXISTS 'fluxer'`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "user" DROP CONSTRAINT IF EXISTS "UQ_user_fluxerId"`,
    );
    await queryRunner.query(`
      ALTER TABLE "user"
