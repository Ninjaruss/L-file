...
export async function checkDatabaseConsistency() {
  ...
  try {
    ...
    if (!existsSync(typeormConfigPath)) {
      console.log(chalk.yellow('⚠️ TypeORM config file not found at ' + typeormConfigPath));
      return false;
    }
    ...
    const cliEntitiesString = JSON.stringify(typeormCliConfig.options?.entities);
    
    if (!appEntitiesString.includes('entities') || !cliEntitiesString.includes('entities')) {
    ...
    const cliMigrationsString = JSON.stringify(typeormCliConfig.options?.migrations);
    
    if (!appMigrationsString.includes('migrations') || !cliMigrationsString.includes('migrations')) {
      issues.push('Migration path configuration mismatch');
    }
    
    // Report issues if any
    if (issues.length > 0) {
      console.log(chalk.yellow('⚠️ Database configuration inconsistencies found:'));
      issues.forEach(issue => console.log(chalk.yellow(`  - ${issue}`)));
      console.log(chalk.yellow('Please ensure both configs match to prevent migration issues.'));
      return false;
    }
...
export async function performDatabaseSafetyChecks(): Promise<void> {
  ...
  if (process.env.NODE_ENV === 'test') {
    console.log(chalk.blue('🧪 Test environment detected, skipping safety checks'));
    return;
  }
  ...
  if (process.env.NODE_ENV === 'production') {
    if (process.env.ENABLE_SCHEMA_SYNC === 'true') {
      console.error(chalk.red('❌ CRITICAL RISK: Schema synchronization is enabled in production!'));
      console.error(chalk.red('This could cause data loss. Please disable ENABLE_SCHEMA_SYNC in production.'));
      process.exit(1);
    }
    
    // When in production, ensure migrations exist before starting
    if (!checkExistingMigrations()) {
      console.warn(chalk.yellow('⚠️ No migrations found but running in production mode.'));
      console.warn(chalk.yellow('It is recommended to use migrations in production for controlled schema updates.'));
    }
  }
  ...
  if (!isConsistent && process.env.NODE_ENV !== 'development') {
    console.error(chalk.red('❌ Database configuration inconsistency detected.'));
    console.error(chalk.red('This could lead to unexpected behavior with migrations.'));
