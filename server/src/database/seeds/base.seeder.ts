...
export abstract class BaseSeeder {
  ...
  public async seed(): Promise<boolean> {
    ...
    try {
      ...
      if (!shouldRun) {
        this.logger.log(chalk.yellow(`⏭️ Skipping ${this.constructor.name} - data already exists`));
        return true;
      }
      ...
      this.logger.log(chalk.green(`✅ ${this.constructor.name} completed successfully`));
      return true;
    } catch (error) {
      ...
      if (error.parameters) {
        this.logger.error(chalk.red(`Parameters: ${JSON.stringify(error.parameters)}`));
      }
