...
async function scrape() {
  ...
  type VolumeMapEntry = { number: number; startChapter: number; endChapter?: number; coverUrl?: string; description?: string; __linkToFollow?: string };
  ...
  $('#mw-content-text').find('li, tr, td').each((i, el) => {
    ...
    const match = text.match(/(?:Chapter\s*)?(\d{1,4})[^\d\n]*[-–—:\.]?\s*(.*)$/i);
    if (match) {
  ...
  $('#mw-content-text').find('h2, h3, .thumb, .portable-infobox, .pi-image').each((i, el) => {
    ...
    if (volMatch) {
      ...
      const imgUrl = img && (img.attr('src') || img.attr('data-src') || img.attr('data-image-name'));
      if (imgUrl) volumesMap[num].coverUrl = imgUrl;
        ...
        const link = $(el).find('a[href*="/wiki/"]').first() || $(el).nextAll('a[href*="/wiki/"]').first();
        ...
        if (href) {
          ...
          if (!volumesMap[num].coverUrl) volumesMap[num].__linkToFollow = full;
        }
      }
    }
  });
  ...
  if (Object.keys(volumesMap).length === 0 && chapters.length) {
    ...
    for (const c of chapters) {
      ...
      if (!volumesMap[volIndex]) volumesMap[volIndex] = { number: volIndex, startChapter: c.number, endChapter: c.number };
      volumesMap[volIndex].endChapter = c.number;
    }
  } else {
    // fill start/end chapter for detected volumes using the chapter list
    const volNumbers = Object.keys(volumesMap).map((k) => parseInt(k, 10)).sort((a, b) => a - b);
    for (let i = 0; i < volNumbers.length; i++) {
      const vnum = volNumbers[i];
      const start = chapters.find((c) => c.number >= 1 && c.number <= 999 && ((i === 0) || c.number > (volumesMap[volNumbers[i - 1]].endChapter || 0)));
      ...
      if (i < volNumbers.length - 1) {
        ...
        const nextStart = chapters.find((c) => c.number >= 1 && c.number <= 999 && c.number >= volumesMap[nextVol].startChapter);
        if (nextStart) volumesMap[vnum].endChapter = nextStart.number - 1;
      } else {
        volumesMap[vnum].endChapter = chapters.length ? chapters[chapters.length - 1].number : undefined;
      }
    }
  }

  const volumes = Object.values(volumesMap).map((v) => ({ number: v.number, startChapter: v.startChapter, endChapter: v.endChapter || null, coverUrl: v.coverUrl || null, description: v.description || null }));

  console.log(`Found ${chapters.length} chapters and ${volumes.length} volumes (heuristic).`);

  writeJson('chapters.json', chapters);
  // If we have volume link placeholders to follow, fetch those pages to find images
  const volumesWithLinks = Object.values(volumesMap).filter((v: VolumeMapEntry) => !!v.__linkToFollow) as VolumeMapEntry[];
  for (const v of volumesWithLinks) {
    try {
      ...
      const candidate = $$('table.portable-infobox img').first() || $$('.thumb img').first() || $$('.pi-image img').first();
      const candUrl = candidate && (candidate.attr('src') || candidate.attr('data-src'));
      if (candUrl) {
